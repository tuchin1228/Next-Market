            ...
            <article className='min-h-screen mt-20 relative'>
                ...
                <div className="category">
                    <div className=' flex flex-wrap md:flex-nowrap justify-center items-center my-16'>
                        <Link href={'news'}><a className=' w-4/5 md:w-auto text-center my-1 px-6 lg:px-12 py-2 bg-yellow-900 hover:bg-yellow-900 transition-all duration-200 text-white text-lg mx-3 rounded-full tracking-widest'>所有公告</a></Link>
                        {
                            categories.map(item => {
                                return <Link href={'news/' + item.id} key={item.id}><a className=' w-4/5 md:w-auto text-center my-1 px-6 lg:px-12 py-2 bg-gray-300 hover:bg-yellow-900 transition-all duration-200 text-white text-lg mx-3 rounded-full tracking-widest'>{item.articles_cate_title}</a></Link>
                            })
                        }
                    </div>
